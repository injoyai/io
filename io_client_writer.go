...
func (this *Client) GoTimerWriter(interval time.Duration, write func(w *Client) error) {
...
func (this *Client) GoTimerWriteBytes(interval time.Duration, p []byte) {
	this.GoTimerWriter(interval, func(w *Client) error {
		_, err := w.Write(p)
		return err
	})
}
...
func (this *Client) SetKeepAlive(t time.Duration, keeps ...[]byte) {
	this.GoTimerWriter(t, func(c *Client) error {
